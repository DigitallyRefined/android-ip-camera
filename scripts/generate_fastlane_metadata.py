...
def ensure_gradle_wrapper_integrity(repo_root):
    wrapper_props_path = repo_root / "gradle" / "wrapper" / "gradle-wrapper.properties"
    if not wrapper_props_path.exists():
        return

    content = read_file(wrapper_props_path)

    print("Checking Gradle wrapper integrity...")

    dist_url_match = re.search(r'distributionUrl=(.+)', content)
    if not dist_url_match:
        print("Could not find distributionUrl in gradle-wrapper.properties")
        return

    dist_url = dist_url_match.group(1).strip().replace("\\:", ":")
    sha256_url = dist_url + ".sha256"

    try:
        print(f"Fetching SHA256 from {sha256_url}...")
        with urllib.request.urlopen(sha256_url) as response:
            sha256_sum = response.read().decode("utf-8").strip()

        if len(sha256_sum) == 64:
            if "distributionSha256Sum=" in content:
                new_content = re.sub(r'distributionSha256Sum=.*', f'distributionSha256Sum={sha256_sum}', content)
                if new_content != content:
                    wrapper_props_path.write_text(new_content, encoding="utf-8")
                    print(f"Updated distributionSha256Sum to {sha256_sum}")
                else:
                    print("distributionSha256Sum is already up to date.")
            else:
                new_content = content.strip() + f"\ndistributionSha256Sum={sha256_sum}\n"
                wrapper_props_path.write_text(new_content, encoding="utf-8")
                print(f"Added distributionSha256Sum={sha256_sum}")
        else:
            print(f"Invalid SHA256 checksum received: {sha256_sum}")
    except Exception as e:
        print(f"Failed to fetch or update Gradle SHA256: {e}")

def main():
    repo_root = Path(os.getenv("GITHUB_WORKSPACE", Path(__file__).resolve().parents[1]))

    ensure_gradle_wrapper_integrity(repo_root)
